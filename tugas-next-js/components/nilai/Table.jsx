...
const FootballerTable = ({nilai})=>{
  ...
  const fetchData = async ()=>{
    let res = await axios.get(`${baseUrl}/nilai`)
  ...
  const handleDelete = async(deletedId)=>{
    ...
    if (confirm(text) == true) {
      try{
        await axios.delete(`${baseUrl}/footballers/${deletedId}`, {headers: {Authorization: `Bearer ${user.token}`}})
        setFetchStatus(true)
      }catch(err){
        alert(err)
      }
  ...
  return (
    <div className=" w-9/12 mx-auto mt-[50px]">
      <h1 className="text-[20px] text-center font-bold mb-[50px]">Footballer List</h1>
